...
class UserLearningRecordControllerTest {
    ...
    void getByUserId() {
        when(recordService.getByUserId("user-1")).thenReturn(List.of(
                UserLearningRecordVO.builder().id("r-1").build()
        ));
    ...
    void getByUserIdAndChapterId() {
        when(recordService.getByUserIdAndChapterId("user-1", "ch-1")).thenReturn(List.of(
                UserLearningRecordVO.builder().id("r-1").build()
        ));
    ...
    void create() {
        UserLearningRecordDto dto = UserLearningRecordDto.builder().userId("user-1").chapterId("ch-1").build();
