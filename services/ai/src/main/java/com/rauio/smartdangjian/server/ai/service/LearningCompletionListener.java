 ...
 *       同一用户同一章节 24 小时内去抖；</li>
 *   <li>自动生成本次学习评估：经 assessment 路由的 Agent 生成评估报告，
 *       会话随 {@code AiMemoryService#saveConversation} 自动存档供用户回看。</li>
 * </ul>
 */
@Slf4j
@Service
...
public class LearningCompletionListener {
    ...
    void generateEvaluation(LearningCompletedEvent event) {
        ...
        AiChatRequest request = new AiChatRequest("learning-eval-" + event.userId() + "-" + event.chapterId(), prompt);
