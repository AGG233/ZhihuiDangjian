...
public class LLMService {
    ...
    public LLMService(AiAgentRegistry aiAgentRegistry, UserService userService, AiMemoryService aiMemoryService) {
            ...
            try {
                return Flux.concat(
                                ...
                                aiAgentRegistry.getCoordinator().stream(input, runnableConfig)
                                        ...
                                        .onErrorResume(e -> Flux.just(new AiChatResponse(
                                                AiChatResponseType.ERROR,
                                                sessionId,
                                                "AI 服务暂时不可用，请稍后重试",
                                                "error",
                        ...
                        .concatWith(Flux.just(
                                new AiChatResponse(AiChatResponseType.END, sessionId, "", "end", "coordinator")));
    ...
    private static AiChatResponse buildAiChatResponse(
            String sessionId, String agentName, NodeOutput output, AtomicReference<String> finalOutput) {
        ...
        if (output instanceof StreamingOutput streamingOutput) {
            ...
            if (type == OutputType.AGENT_MODEL_FINISHED && message instanceof AssistantMessage am) {
                if (am.hasToolCalls()) {
                    return new AiChatResponse(
                            AiChatResponseType.TOOL_CALL,
                            sessionId,
                            am.getToolCalls().toString(),
                            output.node(),
                            agentName);
                }
                finalOutput.set(am.getText());
                return new AiChatResponse(
                        AiChatResponseType.FINISHED, sessionId, am.getText(), output.node(), agentName);
            }

            if (type == OutputType.AGENT_TOOL_FINISHED && message instanceof ToolResponseMessage tr) {
                return new AiChatResponse(
                        AiChatResponseType.TOOL_RESULT,
                        sessionId,
                        tr.getResponses().toString(),
                        output.node(),
                        agentName);
            }
        }
