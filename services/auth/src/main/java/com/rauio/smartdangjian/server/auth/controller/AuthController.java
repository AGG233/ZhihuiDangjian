...
public class AuthController {
    ...
    @Operation(summary = "行为验证码图片", description = "uuid为验证码的唯一标识码，base64为图片的base64形式，需要转换为图片")
    @GetMapping("/captcha")
    ...
    @Operation(summary = "行为验证码验证接口", description = "uuid为验证码的唯一标识码，code为用户输入的验证码，验证成功返回true，验证失败返回false")
    @PostMapping("/captcha")
    ...
    @Operation(
            summary = "统一用户登录",
            description =
                    "需要人机验证（获取验证码以及校验验证码是否有误），登录成功后将返回jwt令牌，大部分接口访问都需要在请求头的Authorization字段添加如下格式：Bearer <替换为jwt令牌>")
    @PostMapping("/login")
