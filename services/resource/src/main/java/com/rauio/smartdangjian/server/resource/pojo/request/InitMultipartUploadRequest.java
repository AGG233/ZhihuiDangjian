...
public record InitMultipartUploadRequest(
        @Schema(description = "文件内容哈希，通常由前端预先计算，用于秒传判定和资源去重", example = "8f14e45fceea167a5a36dedd4bea2543")
                ...
                String fileHash,
        @Schema(description = "文件原始名称，包含文件名本体，通常不要求包含完整路径", example = "lesson.mp4") @NotBlank(message = "fileName不能为空")
                String fileName,
        @Schema(description = "文件后缀名，服务端会进一步标准化处理，可传mp4或.mp4", example = "mp4") @NotBlank(message = "suffix不能为空")
                String suffix,
        @Schema(description = "文件MIME类型，用于COS元数据和资源类型记录", example = "video/mp4") @NotBlank(message = "contentType不能为空")
                String contentType,
