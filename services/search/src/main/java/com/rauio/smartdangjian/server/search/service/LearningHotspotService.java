...
public class LearningHotspotService {
    ...
    public List<HotCategoryResponse> getHotCategories(int limit) {
        ...
        if (rawList.isEmpty()) return Collections.emptyList();
        return rawList.stream().map(raw -> HotCategoryResponse.builder()
    ...
    public LearningTrendResponse getTrends(int days) {
        ...
        Map<String, Integer> dateCountMap = rawList.stream()
                .collect(Collectors.toMap(TrendRaw::getDate, TrendRaw::getCount));
        ...
        for (int i = days - 1; i >= 0; i--) {
            ...
            dailyData.add(LearningTrendResponse.DailyCount.builder().date(dateStr).count(count).build());
        }
        double avgDailyCount = days > 0 ? (double) totalCount / days : 0;
        return LearningTrendResponse.builder()
                .days(days).totalCount(totalCount).avgDailyCount(avgDailyCount).dailyData(dailyData).build();
    }
